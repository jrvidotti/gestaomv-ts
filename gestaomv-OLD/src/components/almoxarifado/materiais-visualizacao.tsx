...
import { STATUS_SOLICITACAO, StatusSolicitacaoType } from '@/shared';
...
export function MateriaisVisualizacao({
  ...
  const podeEditarQtd =
    podeEditar &&
    solicitacaoStatus &&
    (solicitacaoStatus === STATUS_SOLICITACAO.PENDENTE || solicitacaoStatus === STATUS_SOLICITACAO.APROVADA);

  const atualizarQtdAprovadorMutation = api.almoxarifado.atualizarQtdAtendidaAprovador.useMutation({
    onSuccess: () => {
  ...
  const atualizarQtdGerenteMutation = api.almoxarifado.atualizarQtdAtendidaGerente.useMutation({
    onSuccess: () => {
      setEditandoItem(null);
      onAtualizacao?.();
    },
        ...
        <CardTitle className="flex items-center justify-between">
          ...
          {mostrarValorTotal && (
            <div className="text-right">
              ...
              <p className="text-lg font-bold text-primary">{formatCurrency(calcularValorTotal())}</p>
            </div>
          )}
        </CardTitle>
        {!mostrarValorTotal && (
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <span className="text-xs text-muted-foreground">
                        {formatCurrency(material.valorUnitario)}/{material.unidadeMedida?.nome || 'unid'}
                      </span>
                    </div>
                  </div>
                </div>
                    ...
                    {estaEditando ? (
                      <div className="flex items-center gap-1">
                        <Input
                          ...
                          type="number"
                          min="0"
                          disabled={atualizarQtdAprovadorMutation.isPending || atualizarQtdGerenteMutation.isPending}
                        />
                        <Button
                          size="sm"
                          variant="ghost"
                        ...
                        <div className="text-right">
                          <div className="flex items-center gap-1">
                            <span className="text-sm font-medium">
                              {qtdAtual} {material.unidadeMedida?.nome || 'unid'}
                            </span>
                          ...
                          {material.qtdAtendida !== undefined && material.qtdAtendida !== material.qtdSolicitada && (
                            <span className="text-xs text-muted-foreground">
