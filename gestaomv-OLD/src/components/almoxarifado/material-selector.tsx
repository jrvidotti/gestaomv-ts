...
import { Search, Package, Plus, Minus, Filter, X } from 'lucide-react';
...
export function MaterialSelector({ materiais, onChange, disabled = false }: MaterialSelectorProps) {
  ...
  const { data: materiaisDisponiveis, isLoading: carregandoMateriais } = api.almoxarifado.listarMateriais.useQuery({
    ativo: true,
  ...
  const atualizarQuantidade = (materialId: number, novaQuantidade: number) => {
    ...
    onChange(materiais.map((m) => (m.materialId === materialId ? { ...m, qtdSolicitada: novaQuantidade } : m)));
  };
            ...
            <div className="flex items-center justify-between">
              ...
              {materiais.length > 0 && (
                <div className="text-sm text-muted-foreground">
                  {materiais.length} {materiais.length === 1 ? 'material selecionado' : 'materiais selecionados'} /
                  Total: {formatCurrency(materiais.reduce((total, m) => total + m.valorUnitario * m.qtdSolicitada, 0))}
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col gap-3 md:flex-row md:items-end">
              ...
              <div className="min-w-[200px]">
                <LookupSelect
                  ...
                  emptyMessage={isLoadingTiposMaterial ? 'Carregando...' : 'Todos os tipos'}
                  disabled={disabled || isLoadingTiposMaterial}
                />
              </div>
              <Button variant="outline" type="button" onClick={limparFiltros} disabled={disabled} size="sm">
                Limpar
              </Button>
            </div>
          </div>
        ...
        <CardContent>
          {carregandoMateriais ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-muted-foreground">Tente ajustar os filtros para encontrar materiais.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  ...
                  >
                    <div className="flex gap-3">
                      ...
                      <div className="flex-1 min-w-0 pr-6">
                        <h4 className="font-medium text-sm mb-1 truncate" title={material.nome}>
                          {material.nome}
                        </h4>
                        <div className="space-y-1">
                          ...
                          <div className="text-xs text-muted-foreground">
                            <p>
                              {formatCurrency(material.valorUnitario)}/{material.unidadeMedida?.nome}
                            </p>
                          </div>
                        </div>
                      </div>
                        ...
                        <div className="flex justify-between items-center">
                          <div className="flex items-center gap-2">
                            <Button
                              ...
                              size="sm"
                              onClick={() =>
                                atualizarQuantidade(material.id, (materialSelecionado?.qtdSolicitada || 1) - 1)
                              }
                              disabled={disabled}
                          ...
                          {selecionado && (
                            <>
                              <p className="font-medium">
                                {formatCurrency(material.valorUnitario * (materialSelecionado?.qtdSolicitada || 1))}
                              </p>
                              <Button
                                type="button"
                                variant="ghost"
