...
export function RoleBadge({ role, className }: RoleBadgeProps) {
  ...
  return (
    <Badge variant={getVariant(role)} className={className} title={roleData.description}>
      {roleData.label}
    </Badge>
  );
}

export function MultipleRoleBadges({ roles, maxDisplay = 2, className }: MultipleRoleBadgeProps) {
