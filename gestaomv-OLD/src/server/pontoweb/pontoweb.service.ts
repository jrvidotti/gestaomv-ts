...
import { ResultadoImportacao } from '@/shared';
...
export class PontowebService {
  ...
  private validarCredenciais(): void {
    if (!process.env.PONTOWEB_USER || !process.env.PONTOWEB_PASS) {
      throw new Error('Variáveis de ambiente PONTOWEB_USER e PONTOWEB_PASS devem estar configuradas');
    }
  }
  ...
  private async importarEmpresa(empresaPontoWeb: any): Promise<{ id: number; acao: 'criada' | 'atualizada' }> {
    // Verificar se empresa já existe por pontowebId
    const empresaExistente = await this.empresasService.findByPontowebId(empresaPontoWeb.Id);

    const dadosEmpresa = {
      razaoSocial: empresaPontoWeb.Nome,
      nomeFantasia: empresaPontoWeb.Nome,
      cnpj: empresaPontoWeb.Documento || `${empresaPontoWeb.Id}.000.000/0001-00`,
      pontowebId: empresaPontoWeb.Id,
    };

    if (empresaExistente) {
      // Atualizar empresa existente
      const empresaAtualizada = await this.empresasService.update(empresaExistente.id, dadosEmpresa);
      return { id: empresaAtualizada!.id, acao: 'atualizada' };
    } else {
      // Criar nova empresa
  ...
  private async importarUnidade(estruturaPontoWeb: any): Promise<{ id: number; acao: 'criada' | 'atualizada' }> {
    // Verificar se unidade já existe por pontowebId
    const unidadeExistente = await this.unidadesService.findByPontowebId(estruturaPontoWeb.Id);
    ...
    if (unidadeExistente) {
      ...
      const unidadeAtualizada = await this.unidadesService.update(unidadeExistente.id, dadosUnidade);
      return { id: unidadeAtualizada!.id, acao: 'atualizada' };
    } else {
      // Criar nova unidade
  ...
  private async importarDepartamento(
    ...
    const departamentoExistente = await this.departamentosService.buscarDepartamentoPorPontowebId(
    ...
    if (departamentoExistente) {
      ...
      const departamentoAtualizado = await this.departamentosService.atualizarDepartamento(
        departamentoExistente.id,
        dadosDepartamento,
      );
      return { id: departamentoAtualizado!.id, acao: 'atualizado' };
    } else {
      // Criar novo departamento
      const novoDepartamento = await this.departamentosService.criarDepartamento(dadosDepartamento);
  ...
  private async importarCargo(
    ...
    const cargoExistente = await this.cargosService.buscarCargoPorPontowebId(funcaoPontoWeb.Id);
    ...
    if (cargoExistente) {
      ...
      const cargoAtualizado = await this.cargosService.atualizarCargo(cargoExistente.id, dadosCargo);
      return { id: cargoAtualizado!.id, acao: 'atualizado' };
    } else {
      // Criar novo cargo
  ...
  private async importarFuncionario(
    ...
    modoAtualizar: boolean = false,
  ): Promise<'importado' | 'atualizado' | 'ignorado' | 'erro'> {
    try {
      // Verificar se o funcionário já existe
      const funcionarioExistente = await this.funcionariosService.buscarFuncionarioPorPontowebId(funcionario.Id);

      // Mapear IDs das estruturas do PontoWeb para IDs do banco local
      const empresaId = (funcionario as any).EmpresaId ? empresasMap.get((funcionario as any).EmpresaId) : null;
      const unidadeId = (funcionario as any).EstruturaId ? unidadesMap.get((funcionario as any).EstruturaId) : null;
      const departamentoId = (funcionario as any).DepartamentoId
        ? departamentosMap.get((funcionario as any).DepartamentoId)
        : null;
      const cargoId = (funcionario as any).FuncaoId ? cargosMap.get((funcionario as any).FuncaoId) : null;

      const dadosFuncionario = {
        nome: funcionario.Nome,
        cpf: funcionario.Cpf,
        dataNascimento: funcionario.Nascimento ? funcionario.Nascimento.toISOString() : undefined,
        sexo: undefined as 'M' | 'F' | 'Outro' | undefined,
        nomeMae: undefined,
        email: funcionario.Email || undefined,
        telefone: funcionario.Telefone || (funcionario as any).Celular || undefined,
        foto: undefined,
        ...
        dataAdmissao: funcionario.Admissao!.toISOString(),
        dataAvisoPrevio: undefined,
        dataDesligamento: funcionario.Demissao ? funcionario.Demissao.toISOString() : undefined,
        status: funcionario.Demissao ? ('DESLIGADO' as const) : ('ATIVO' as const),
      ...
      if (funcionarioExistente) {
        if (modoAtualizar) {
          ...
          await this.funcionariosService.atualizarFuncionario(funcionarioExistente.id, dadosFuncionario);
          return 'atualizado';
        } else {
  ...
  async importarFuncionariosPontoWeb(modoAtualizar: boolean = false): Promise<ResultadoImportacao> {
    ...
    try {
      ...
      const client = await PontoWebClient.init(process.env.PONTOWEB_USER!, process.env.PONTOWEB_PASS!);
      ...
      console.log(`📊 Encontrados ${funcionarios.length} funcionários no PontoWeb`);
      ...
      for (const funcionario of funcionarios) {
        ...
        if (func.Departamento && !departamentosUnicos.has(func.Departamento.Id)) {
      ...
      for (const [id, departamento] of departamentosUnicos) {
        try {
          const { id: departamentoId, acao } = await this.importarDepartamento(departamento);
      ...
      console.log(`📊 Encontrados ${cargosUnicos.size} cargos únicos para importar`);
      ...
      for (const [id, cargo] of cargosUnicos) {
        try {
          ...
          const func = cargo;
          let departamentoId = func.DepartamentoId ? departamentosMap.get(func.DepartamentoId) : null;

          console.log(`   Departamento original: ${func.DepartamentoId}, Mapeado: ${departamentoId}`);

          // Se não há departamento mapeado, usar/criar um departamento padrão
          if (!departamentoId) {
            if (!departamentoPadrao) {
              console.log('🏛️ Criando departamento padrão para cargos sem departamento...');
              try {
                departamentoPadrao = await this.departamentosService.criarDepartamento({
                  nome: 'Departamento Geral',
                  descricao: 'Departamento padrão para cargos importados do PontoWeb sem departamento específico',
                });
                console.log(`✅ Departamento padrão criado com ID: ${departamentoPadrao.id}`);
              } catch (error) {
                // Se já existe, buscar por nome
                console.log('🔍 Buscando departamento padrão existente...');
                const departamentos = await this.departamentosService.listarDepartamentos({
                  busca: 'Departamento Geral',
                  limite: 1,
                  pagina: 1,
                });
                if (departamentos.departamentos.length > 0) {
                  departamentoPadrao = { id: departamentos.departamentos[0].id };
                  console.log(`✅ Departamento padrão encontrado com ID: ${departamentoPadrao.id}`);
                } else {
                  throw error;
                }
              }
          ...
          const { id: cargoId, acao } = await this.importarCargo(cargo, departamentoId);
      ...
      for (const funcionario of funcionarios) {
        ...
        switch (resultadoFuncionario) {
          case 'importado':
            ...
            console.log(`✅ Importado: ${funcionario.Nome} (CPF: ${funcionario.Cpf})`);
            break;
          case 'atualizado':
            resultado.funcionarios.atualizados++;
            console.log(`🔄 Atualizado: ${funcionario.Nome} (CPF: ${funcionario.Cpf})`);
            break;
          case 'ignorado':
            resultado.funcionarios.ignorados++;
            console.log(`ℹ️ Ignorado (já existe): ${funcionario.Nome} (CPF: ${funcionario.Cpf})`);
            break;
          case 'erro':
      ...
      console.log(`      • Cargos: ${resultado.cargos.importados} novos, ${resultado.cargos.atualizados} atualizados`);
  ...
  async sincronizarAfastamentos(diasRetroativos: number = 30): Promise<{
    afastamentos: number;
    erros: string[];
  }> {
    const resultado: {
    ...
    try {
      ...
      console.log(`📋 Buscando afastamentos dos últimos ${diasRetroativos} dias...`);
      const afastamentos = await client.listaAfastamentos(diasRetroativos);

      console.log(`📊 Encontrados ${afastamentos.length} afastamentos no PontoWeb`);
  ...
  async obterMotivosDemissao(): Promise<{
    motivos: Array<{ id: number; descricao: string; codigo?: string; ativo: boolean }>;
    erros: string[];
  }> {
    const resultado: {
      motivos: Array<{ id: number; descricao: string; codigo?: string; ativo: boolean }>;
    ...
    try {
      ...
      console.log(`✅ Encontrados ${resultado.motivos.length} motivos de demissão!`);
    } catch (error) {
