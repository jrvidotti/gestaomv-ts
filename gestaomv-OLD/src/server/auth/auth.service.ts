...
export class AuthService {
  ...
  async register(registerDto: RegisterDto): Promise<AuthResponse> {
    ...
    const { password, ...userWithoutPassword } = user;
    this.notificationsService.notifyNewUserRegistration(userWithoutPassword).catch((error) => {
  ...
  async addUserRole(addUserRoleDto: AddUserRoleDto): Promise<void> {
    return await this.usersService.addUserRole(addUserRoleDto.userId, addUserRoleDto.role);
  }

  async removeUserRole(removeUserRoleDto: RemoveUserRoleDto): Promise<void> {
    return await this.usersService.removeUserRole(removeUserRoleDto.userId, removeUserRoleDto.role);
  }
  ...
  async loginWithTagOne(loginDto: TagOneAuthLoginDto): Promise<AuthResponse> {
    ...
    const tagoneResult = await this.tagoneService.loginWithTagOne(loginDto.username, loginDto.password);
    ...
    if (tagoneEmail && this.isValidEmail(tagoneEmail)) {
      ...
      const existingEmailUser = await this.usersService.findByEmail(tagoneEmail);
      if (existingEmailUser && existingEmailUser.authProvider !== 'tagone') {
        // Verificar se este usuário já conectou ao TagOne
        const existingTagoneConnection = await this.usersService.findByTagOneUsername(loginDto.username);
        if (!existingTagoneConnection || existingTagoneConnection.id !== existingEmailUser.id) {
          throw new Error(
    ...
    const existingTagoneUser = await this.usersService.findByTagOneUsername(loginDto.username);
