...
import { userFuncionarios, funcionarios, users, CriarUserFuncionarioData, FiltrosUserFuncionarios } from '@/shared';
...
export class UserFuncionariosService {
  ...
  async listarUserFuncionarios(filtros?: FiltrosUserFuncionarios) {
    ...
    const userFuncionariosList = await this.db
      ...
      .leftJoin(funcionarios, eq(userFuncionarios.funcionarioId, funcionarios.id))
      .where(whereClause)
      .limit(limite)
      .offset(offset);

    const [{ total }] = await this.db.select({ total: count() }).from(userFuncionarios).where(whereClause);
  ...
  async buscarUserPorFuncionario(funcionarioId: string) {
    const userFuncionario = await this.buscarUserFuncionarioPorFuncionarioId(funcionarioId);
    return userFuncionario?.user || null;
  }

  async verificarVinculoExiste(userId: string, funcionarioId: string): Promise<boolean> {
    const vinculo = await this.db.query.userFuncionarios.findFirst({
      where: and(eq(userFuncionarios.userId, userId), eq(userFuncionarios.funcionarioId, funcionarioId)),
    });

    return !!vinculo;
  }

  async deletarUserFuncionario(userId: string, funcionarioId: string): Promise<void> {
    await this.db
  ...
  async deletarVinculoPorUserId(userId: string): Promise<void> {
    await this.db.delete(userFuncionarios).where(eq(userFuncionarios.userId, userId));
  }

  async deletarVinculoPorFuncionarioId(funcionarioId: string): Promise<void> {
    await this.db.delete(userFuncionarios).where(eq(userFuncionarios.funcionarioId, funcionarioId));
  }
}
