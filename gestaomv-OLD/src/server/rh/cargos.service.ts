...
import { cargos, departamentos, CriarCargoData, AtualizarCargoData, FiltrosCargos } from '@/shared';
...
export class CargosService {
  ...
  async listarCargos(filtros?: FiltrosCargos) {
    ...
    if (busca) {
      condicoes.push(sql`${cargos.nome} LIKE ${'%' + busca + '%'} OR ${cargos.descricao} LIKE ${'%' + busca + '%'}`);
    }
    ...
    const [{ total }] = await this.db.select({ total: count() }).from(cargos).where(whereClause);
