...
type StatusFuncionario = StatusFuncionarioType;
...
export class FuncionariosService {
  ...
  async listarFuncionarios(filtros?: FiltrosFuncionarios) {
    const { busca, cargoId, departamentoId, empresaId, unidadeId, status, pagina = 1, limite = 20 } = filtros || {};
    ...
    const funcionariosList = await this.db
      ...
      .leftJoin(departamentos, eq(funcionarios.departamentoId, departamentos.id))
    ...
    const [{ total }] = await this.db.select({ total: count() }).from(funcionarios).where(whereClause);
  ...
  async buscarFuncionariosPorStatus(status: StatusFuncionario[]) {
    const funcionariosList = await this.db.query.funcionarios.findMany({
  ...
  async atualizarFuncionario(id: number, funcionarioData: AtualizarFuncionarioData) {
    const dadosAtualizacao = {
      ...funcionarioData,
      atualizadoEm: new Date().toISOString(),
    };

    await this.db.update(funcionarios).set(dadosAtualizacao).where(eq(funcionarios.id, id));
