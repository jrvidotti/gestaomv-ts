...
type UserProfile = NonNullable<Awaited<ReturnType<typeof trpcClient.auth.profile.query>>>;
type LoginInput = Parameters<typeof trpcClient.auth.login.mutate>[0];
type TagOneLoginInput = Parameters<typeof trpcClient.auth.loginWithTagOne.mutate>[0];
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const loginWithTagOne = async (credentials: TagOneLoginInput) => {
    setIsLoading(true);
    try {
      const response = await trpcClient.auth.loginWithTagOne.mutate(credentials);
  ...
  const hasRole = (role: UserRoleType | string): boolean => {
    ...
    return user.roles && user.roles.some((userRole) => userRole.toString() === role);
  };
  ...
  const getAllRoles = (): UserRoleType[] => {
    ...
    if (user.roles) {
      user.roles.forEach((role: UserRoleType) => {
