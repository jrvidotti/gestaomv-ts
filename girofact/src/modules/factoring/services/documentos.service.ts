...
export class DocumentosService {
  ...
  async pagarDocumento(data: PagamentoDto, userId: number) {
    ...
    if (
      ...
      throw new ValidationError(
        "Documento não pode ser pago neste status"
      );
    }
  ...
  async posicaoDocumentos(data: PosicaoDocumentosDto) {
    ...
    const conditions = [
      lte(documentos.criadoEm, dataReferenciaStr),
    ];
    ...
    const agrupados = this.agruparDocumentos(documentosResult, agruparPor, dataReferencia);
  ...
  private agruparDocumentos(documentos: any[], agruparPor: string, dataReferencia: Date) {
    ...
    documentos.forEach((doc) => {
      ...
      switch (agruparPor) {
        ...
        case "cliente":
          ...
          break;
        case "vencimento":
          const diasVencimento = this.calcularDiasVencimento(doc.dataVencimento, dataReferencia);
  ...
  private calcularDiasVencimento(dataVencimento: string, dataReferencia: Date): number {
