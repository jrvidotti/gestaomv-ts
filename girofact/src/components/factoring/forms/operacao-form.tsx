...
const operacaoFormSchema = z.object({
  ...
  taxaJuros: z.number().min(0.01, "Taxa de juros deve ser maior que 0").max(100, "Taxa não pode ser maior que 100%"),
  valorLiquido: z.number().min(0.01, "Valor líquido deve ser maior que 0").optional(),
...
interface OperacaoFormProps {
  initialData?: Partial<OperacaoFormData & {
    uid: string;
    status: string;
  }>;
...
export function OperacaoForm({
  ...
  const clienteSelecionado = clientes?.find(c => c.id === form.watch("clienteId"));
        ...
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            {initialData?.status && mode === "edit" && (
              <Badge 
                variant={
                  initialData.status === "rascunho" ? "secondary" :
                  initialData.status === "aprovacao" ? "outline" :
                  initialData.status === "efetivada" ? "default" :
                  initialData.status === "liquidada" ? "default" :
                  "destructive"
                }
              >
                        ...
                        >
                          ...
                          <SelectContent>
                            {carteiras?.map((carteira) => (
                              ...
                              >
                                <div>
                                  <div className="font-medium">{carteira.nome}</div>
                                  <div className="text-sm text-muted-foreground">
                  ...
                  <div className="p-4 bg-muted rounded-lg">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                      <div>
                        ...
                        <span className="ml-2">{clienteSelecionado.pessoa.nomeRazaoSocial}</span>
                      </div>
                      <div>
                        <span className="text-muted-foreground">Documento:</span>
                        <span className="ml-2 font-mono">{clienteSelecionado.pessoa.documento}</span>
                      </div>
                      {clienteSelecionado.pessoa.email && (
                        <div>
                          <span className="text-muted-foreground">E-mail:</span>
                          <span className="ml-2">{clienteSelecionado.pessoa.email}</span>
                        </div>
                      )}
                    </div>
                  </div>
