...
export function TelefonesTable({
	...
	const handleSave = (index: number) => {
		...
		// Validação adicional - máximo 11 dígitos
		if (digits.length > 11) {
			alert("Telefone não pode ter mais de 11 dígitos");
			return;
		}
	...
	const getPhoneMask = (value: string) => {
		...
		if (digits.length === 11 || (digits.length >= 3 && digits[2] === "9")) {
									...
									<TableCell>
										{telefone.isEditing ? (
											<MaskedInput
												...
												onChange={(value) =>
													handleFieldChange(index, "numero", value)
												}
