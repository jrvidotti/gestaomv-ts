...
export function DadosBancariosTable({ 
  ...
  const handleFieldChange = (index: number, field: keyof DadosBancariosDto, value: any) => {
    const updatedDados = [...dadosBancarios];
    updatedDados[index] = {
      ...updatedDados[index],
      [field]: value,
  ...
  const formatBankAccount = (banco: string, agencia: string, conta: string, dv: string) => {
                ...
                <TableRow key={dadoBancario.id || index}>
                  ...
                  <TableCell>
                    {dadoBancario.isEditing ? (
                      <Input
                        ...
                        onChange={(e) =>
                          handleFieldChange(index, "digitoVerificador", e.target.value)
                        }
                        placeholder="9"
                        className="w-16"
                        maxLength={2}
                      ...
                      <span className="font-mono">{dadoBancario.digitoVerificador}</span>
                    )}
                  </TableCell>
                  ...
                  <TableCell>
                    {dadoBancario.isEditing ? (
                      ...
                      <Badge variant={dadoBancario.tipoConta === "corrente" ? "default" : "secondary"}>
                        {dadoBancario.tipoConta === "corrente" ? "Corrente" : "Poupança"}
                      </Badge>
                    )}
                  </TableCell>
