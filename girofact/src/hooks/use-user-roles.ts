...
export function useUserRoles({ userId, enabled = true }: UseUserRolesOptions) {
  ...
  const {
    ...
    trpc.core.auth.buscarUserRoles.queryOptions(
      { userId },
  ...
  const { data: userData, isLoading: isLoadingUser } = useQuery(
    trpc.core.users.buscar.queryOptions(
      { id: userId },
      { enabled: enabled && !!userId }
    )
  );
  ...
  const addRoleMutation = useMutation(
    trpc.core.auth.adicionarUserRole.mutationOptions({
      onSuccess: () => {
  ...
  const removeRoleMutation = useMutation(
    trpc.core.auth.removerUserRole.mutationOptions({
      onSuccess: () => {
        queryClient.invalidateQueries({
          queryKey: ["auth.getUserRoles", { userId }],
        });
