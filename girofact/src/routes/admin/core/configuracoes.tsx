...
function ConfiguracoesPage() {
  ...
  const { data: configuracoes, isLoading: configLoading } = useQuery(
    trpc.core.configuracoes.getConfiguracoesSistema.queryOptions()
  );
  const { data: users, isLoading: usersLoading } = useQuery(
    trpc.core.users.listar.queryOptions()
  );

  // Mutation para salvar configurações
  const { mutate: saveConfiguracoes, isPending: isSaving } = useMutation({
    ...trpc.core.configuracoes.updateConfiguracoesSistema.mutationOptions(),
    onSuccess: () => {
