...
import {
	StatsCard,
	StatsCardsGrid,
} from "@/components/factoring/dashboard/stats-cards";
...
function FactoringDashboard() {
	...
	const { data: posicaoDocumentos, isLoading: isPosicaoLoading } = useQuery(
	...
	const { data: carteiraClientes, isLoading: isCarteiraLoading } = useQuery(
	...
	const clientesChartData = [
		...
		{
			nome: "Bloqueados",
			valor: carteiraClientes?.bloqueados || 0,
			cor: "#ef4444",
		},
	];
								...
								<div className="flex items-center justify-between">
									<div>
										...
										<p className="text-2xl font-bold">
											{isPosicaoLoading
												? "..."
												: posicaoDocumentos?.pendentes.quantidade || 0}
										</p>
										<p className="text-xs text-muted-foreground">
											{isPosicaoLoading
												? "..."
												: formatCurrency(
														posicaoDocumentos?.pendentes.valor || 0,
													)}
										</p>
									</div>
								...
								<div className="flex items-center justify-between">
									<div>
										...
										<p className="text-2xl font-bold">
											{isPosicaoLoading
												? "..."
												: posicaoDocumentos?.compensados.quantidade || 0}
										</p>
										<p className="text-xs text-muted-foreground">
											{isPosicaoLoading
												? "..."
												: formatCurrency(
														posicaoDocumentos?.compensados.valor || 0,
													)}
										</p>
									</div>
								...
								<div className="flex items-center justify-between">
									<div>
										...
										<p className="text-2xl font-bold">
											{isPosicaoLoading
												? "..."
												: posicaoDocumentos?.devolvidos.quantidade || 0}
										</p>
										<p className="text-xs text-muted-foreground">
											{isPosicaoLoading
												? "..."
												: formatCurrency(
														posicaoDocumentos?.devolvidos.valor || 0,
													)}
										</p>
									</div>
