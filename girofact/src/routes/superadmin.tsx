...
function SuperadminPage() {
  ...
  const { data: systemInfoData, isLoading: systemLoading } = useQuery(
    trpc.core.superadmin.buscarInfoSistema.queryOptions()
  );
  ...
  } = useQuery(trpc.core.superadmin.buscarStatsSistema.queryOptions());

  const {
    data: migrationInfoData,
    isLoading: migrationLoading,
    refetch: refetchMigrationInfo,
  } = useQuery(trpc.core.superadmin.buscarInfoMigracoes.queryOptions());
  ...
  const runMigrationsMutation = useMutation(
    trpc.core.superadmin.executarMigracoes.mutationOptions({
      onSuccess: (result) => {
        if (result.success) {
  ...
  const runSeedMutation = useMutation(
    trpc.core.superadmin.operacaoSeed.mutationOptions({
      onSuccess: (result) => {
        if (result.success) {
