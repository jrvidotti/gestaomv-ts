...
export default function SuperadminDashboard() {
	...
	const [migrationInfo, setMigrationInfo] = useState<MigrationInfoDTO | null>(
		null,
	);
	...
	const runSeedOperation = async (operation: "import" | "export") => {
		...
		try {
			...
			if (result.success) {
				...
				for (const output of (result as SeedOperationResponseDTO).outputs) {
		...
		<div className="container mx-auto p-6 space-y-6">
			<div className="flex items-center justify-between">
				...
				<Badge variant="secondary">
					Ambiente: {systemInfo?.application.environment}
				</Badge>
			</div>
								...
								<div>
									...
									<div className="text-sm space-y-1">
										...
										<p>
											...
											{systemInfo?.server.memoryUsage.rss}
										</p>
									</div>
								</div>
							</div>
					...
					<Card>
						...
						<CardContent className="space-y-4">
							<div className="flex flex-col lg:flex-row items-center justify-between gap-4">
								<div className="text-sm text-muted-foreground">
									<p>
										...
										| Pendentes:{" "}
										<Badge variant="secondary">
											{migrationInfo?.summary?.pending || 0}
										</Badge>
									</p>
									<p>
										Pasta:{" "}
										<code className="text-xs">
											{migrationInfo?.migrationsFolder}
										</code>
									</p>
								</div>
							...
							{migrationInfo?.migrationsStatus &&
								migrationInfo.migrationsStatus.filter(
									(m) => m.status === "pending",
								).length > 0 && (
