		...
		switch (healthData.status) {
			case "database_missing":
				return {
					...
					description:
						"O banco de dados não existe. É necessário executar as migrações iniciais para criar o banco.",
	...
	return (
		<div className="fixed top-0 left-0 right-0 z-50 shadow-sm">
			<div className="container mx-auto p-4">
				<Card
					className={`border-0 rounded-none ${alertContent.bgColor} ${alertContent.borderColor} border-b`}
				>
					<div className="flex items-center justify-between w-full p-4">
						<div className="flex items-start space-x-3 flex-1">
							<div className={alertContent.textColor}>{alertContent.icon}</div>
							<div className="flex-1 min-w-0">
								<div className={`font-semibold ${alertContent.textColor}`}>
									{alertContent.title}
								</div>
								<div className={`${alertContent.descColor} mt-1 text-sm`}>
					...
					<div className="flex items-center space-x-2 ml-8 gap-2">
						{isLoading && (
							<Loader2
								className={`h-4 w-4 animate-spin ${alertContent.textColor}`}
							/>
						)}
						...
						>
							LinkSuperadmin
							{/* <Link to="/superadmin">Acessar Painel Administrativo</Link> */}
						</Button>
