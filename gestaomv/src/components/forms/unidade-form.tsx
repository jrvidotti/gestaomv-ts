...
import { createUnidadeSchema, updateUnidadeSchema, type CreateUnidadeDto, type UpdateUnidadeDto } from '@/modules/core/dtos'
...
export function UnidadeForm({ mode, initialData, onSubmit, isSubmitting = false }: UnidadeFormProps) {
  const schema = mode === 'create' ? createUnidadeSchema : updateUnidadeSchema
  
  const trpc = useTRPC()
  
  // Buscar empresas para o select
  const { data: empresas, isLoading: empresasLoading } = useQuery(trpc.empresas.findAll.queryOptions())
      ...
      <CardContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="space-y-4">
              <h3 className="text-sm font-medium text-muted-foreground">Contato e Integrações</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <FormField
                  control={form.control}
                  name="telefone"
