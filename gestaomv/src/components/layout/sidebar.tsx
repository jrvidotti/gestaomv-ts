...
import { getSidebarState, setSidebarState } from "@/lib/cookies";
...
function SidebarProvider({
	...
	// Initialize state from cookies on first render to avoid hydration mismatch
	...
	const [_open, _setOpen] = useState(getInitialState);
	const open = openProp ?? _open;

	// Handle hydration
	useEffect(() => {
		if (!isHydrated) {
			const cookieState = getSidebarState();
			if (openProp === undefined && cookieState !== defaultOpen) {
				_setOpen(cookieState);
			}
			setIsHydrated(true);
		}
	}, [defaultOpen, openProp, isHydrated]);
