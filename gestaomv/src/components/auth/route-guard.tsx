    ...
    if (!hasAnyRole(requiredRoles)) {
      return (
        fallback || (
          <Unauthorized
            title={title}
            description={description || `Você precisa ter uma das seguintes permissões: ${requiredRoles.join(', ')}`}
          />
        )
      );
    }
  ...
  if (!canAccessPage(pathname)) {
    return (
      fallback || (
        <Unauthorized
          title={title}
          description={description || 'Você não tem permissão para acessar esta funcionalidade.'}
        />
      )
    );
  }
