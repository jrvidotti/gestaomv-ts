...
import { ClipboardList, Plus, Search, MoreHorizontal, Eye, CheckCircle, XCircle, Package, Filter } from 'lucide-react'
...
import { STATUS_OPTIONS, STATUS_SOLICITACAO_DATA } from '@/modules/almoxarifado/consts'
...
export const Route = createFileRoute('/admin/almoxarifado/solicitacoes/')({
  ...
  validateSearch: (search: Record<string, unknown>): SolicitacoesSearch => {
    return {
      status: typeof search.status === 'string' ? search.status as StatusSolicitacaoType : undefined,
...
function RouteComponent() {
  ...
  const { user } = useAuth()
  const { status: statusUrl, busca: buscaUrl, pagina: paginaUrl } = Route.useSearch()

  const [busca, setBusca] = useState(buscaUrl || '')
  const [statusSelecionado, setStatusSelecionado] = useState<StatusSolicitacaoType | 'all'>(statusUrl || 'all')
  ...
  const { data, isLoading, refetch } = useQuery(trpc.almoxarifado.listarSolicitacoes.queryOptions(filtros))

  const { mutate: aprovarSolicitacao } = useMutation({
    ...trpc.almoxarifado.aprovarOuRejeitarSolicitacao.mutationOptions(),
    onSuccess: () => {
      refetch()
    },
  })
  ...
  const { mutate: atenderSolicitacao } = useMutation({
    ...trpc.almoxarifado.atenderSolicitacao.mutationOptions(),
    onSuccess: () => {
      refetch()
    },
  })
  ...
  const isAdmin = user?.roles?.includes(USER_ROLES.ADMIN) || user?.roles?.includes(USER_ROLES.GERENCIA_ALMOXARIFADO)

  const obterSubtitle = () => {
    let subtitle = 'Acompanhe e gerencie todas as solicitações do almoxarifado'
    if (statusSelecionado && statusSelecionado !== 'all') {
      const statusOption = STATUS_OPTIONS.find((s) => s.value === statusSelecionado)
        ...
        <div className="space-y-6">
              ...
              <div className="flex flex-col gap-4 md:flex-row md:items-end">
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setStatusSelecionado(value as StatusSolicitacaoType)}
                  >
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {isLoading ? 'Carregando...' : `${data?.total || 0} solicitações encontradas`}
              </CardDescription>
            </CardHeader>
            <CardContent>
              {isLoading ? (
                    ...
                    <TableBody>
                      {data.solicitacoes.map((solicitacao) => (
                        <TableRow key={solicitacao.id}>
                          <TableCell className="font-mono">#{solicitacao.id.toString().padStart(6, '0')}</TableCell>
                          <TableCell>
                            <div>
                          ...
                          <TableCell>{solicitacao.unidade?.nome || 'N/A'}</TableCell>
                          <TableCell>
                            <Badge variant={STATUS_SOLICITACAO_DATA[solicitacao.status].variant}>
                              {STATUS_SOLICITACAO_DATA[solicitacao.status].label}
                            </Badge>
                          </TableCell>
                          <TableCell className="text-sm">{formatDateTime(solicitacao.dataOperacao)}</TableCell>
                          <TableCell>
                            <div className="text-sm">
                              <p>{solicitacao.itens?.length || 0} itens</p>
                              <p className="text-muted-foreground">
                                {solicitacao.itens?.reduce((acc, item) => acc + item.qtdSolicitada, 0) || 0} unidades
                              </p>
                            </div>
                          </TableCell>
                          <TableCell>
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end">
                                <Link to="/admin/almoxarifado/solicitacoes/$id" params={{ id: solicitacao.id.toString() }}>
                                  <DropdownMenuItem>
                                    <Eye className="h-4 w-4 mr-2" />
                                    Visualizar
                                  </DropdownMenuItem>
                                </Link>
                                {isAdmin && solicitacao.status === STATUS_SOLICITACAO.PENDENTE && (
                                  <>
                                    <DropdownMenuItem
                                ...
                                {isAdmin && solicitacao.status === STATUS_SOLICITACAO.APROVADA && (
                                  <DropdownMenuItem
