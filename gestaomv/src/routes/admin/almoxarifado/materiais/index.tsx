...
export const Route = createFileRoute('/admin/almoxarifado/materiais/')({
  ...
  validateSearch: (search: Record<string, unknown>): MateriaisSearch => {
    return {
      ...
      tipoMaterialId: typeof search.tipoMaterialId === 'string' ? search.tipoMaterialId : undefined,
...
function RouteComponent() {
  ...
  const { busca: buscaUrl, tipoMaterialId: tipoUrl, pagina: paginaUrl } = Route.useSearch()
  
  const [busca, setBusca] = useState(buscaUrl || '')
  const [tipoSelecionadoId, setTipoSelecionadoId] = useState<string | null>(tipoUrl || null)
  ...
  const { data, isLoading, refetch } = useQuery(trpc.almoxarifado.listarMateriais.queryOptions(filtros))
  const { data: tiposMaterialData, isLoading: isLoadingTiposMaterial } = useQuery(
    trpc.almoxarifado.listarTiposMaterial.queryOptions()
  )

  const { mutate: inativarMaterial } = useMutation({
    ...trpc.almoxarifado.inativarMaterial.mutationOptions(),
    onSuccess: () => {
      refetch()
    },
  })
              ...
              <div className="flex flex-col gap-4 md:flex-row md:items-end">
                ...
                <div>
                  ...
                  <LookupSelect
                    ...
                    emptyMessage={isLoadingTiposMaterial ? 'Carregando...' : 'Nenhum tipo selecionado'}
                    ...
                    <TableBody>
                      {data.materiais.map((material) => (
                        <TableRow key={material.id}>
                          <TableCell>
                            <Link to="/admin/almoxarifado/materiais/$id/edit" params={{ id: material.id.toString() }}>
                          ...
                          <TableCell>
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end">
                                <Link to="/admin/almoxarifado/materiais/$id" params={{ id: material.id.toString() }}>
                                  <DropdownMenuItem>
