...
import { Users, UserCheck, UserPlus, Shield, Plus, Settings } from 'lucide-react'
...
function AdminCoreDashboard() {
  ...
  // Queries tRPC
  const { data: userStats, isLoading: statsLoading } = useQuery(trpc.users.getUserStats.queryOptions())
  const { data: pendingUsers, isLoading: pendingLoading } = useQuery(trpc.users.findPendingUsers.queryOptions())
                      ...
                      <TableRow key={user.id}>
                        ...
                        <TableCell>
                          <Button asChild size="sm">
                            <Link to="/admin/core/users/$id/edit" params={{ id: user.id }}>
                              Aprovar
                            </Link>
                          </Button>
                        </TableCell>
                ...
                {userStats?.pending && userStats.pending > 0 && (
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant="secondary">
                      {userStats.pending} pendente{userStats.pending > 1 ? 's' : ''}
                    </Badge>
                  </div>
                )}
                <div className="flex items-center justify-between">
