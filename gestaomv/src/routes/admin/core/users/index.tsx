...
function UsersListPage() {
  ...
  // Queries tRPC
  const { data: users, isLoading, error, refetch } = useQuery(trpc.users.findAll.queryOptions())
  ...
  const handleToggleStatus = (id: string, currentStatus: boolean) => {
    updateUser({ 
      ...
      onError: () => {
        toast.error(`Erro ao ${!currentStatus ? 'ativar' : 'desativar'} usuário`)
      }
    })
  }
                    ...
                    {filteredUsers.length === 0 ? (
                      ...
                      filteredUsers.map((user) => (
                        <TableRow 
                          ...
                          onClick={() => navigate({ to: '/admin/core/users/$id/edit', params: { id: user.id } })}
                        >
                          <TableCell>
