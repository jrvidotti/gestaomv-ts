...
function EditUserPage() {
  ...
  // Queries tRPC
  const { data: user, isLoading, error } = useQuery(trpc.users.findOne.queryOptions({ id }))
  ...
  const handleRoleChange = async (role: UserRoleType, isActive: boolean) => {
    ...
    updateUser({ 
      ...
      onSuccess: () => {
        toast.success(`Permissão ${isActive ? 'adicionada' : 'removida'} com sucesso!`)
      },
      onError: () => {
        ...
        <div className="max-w-6xl mx-auto space-y-6">
          ...
          {newUser && (
            <Alert>
              ...
              <AlertDescription>
                Usuário criado com sucesso! Agora você pode configurar as permissões e funções necessárias.
              </AlertDescription>
            </Alert>
          )}
