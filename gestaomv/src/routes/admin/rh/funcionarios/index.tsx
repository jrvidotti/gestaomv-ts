...
import { Users, Plus, Search, MoreHorizontal, Pencil, Eye, UserCheck, UserX, Filter } from 'lucide-react'
...
import { STATUS_FUNCIONARIO_OPTIONS, STATUS_FUNCIONARIO_DATA } from '@/modules/rh/consts'
import type { StatusFuncionarioType, FiltrosFuncionarios } from '@/modules/rh/types'
...
export const Route = createFileRoute('/admin/rh/funcionarios/')({
  ...
  validateSearch: (search: Record<string, unknown>): FuncionariosSearch => ({
    ...
    status: typeof search.status === 'string' ? search.status as StatusFuncionarioType : undefined,
...
function RouteComponent() {
  ...
  const { busca: buscaUrl, status: statusUrl, departamentoId: deptoUrl, cargoId: cargoUrl, pagina: paginaUrl } = Route.useSearch()
  
  const [busca, setBusca] = useState(buscaUrl || '')
  const [statusSelecionado, setStatusSelecionado] = useState<StatusFuncionarioType | 'all'>(statusUrl || 'all')
  const [departamentoSelecionado, setDepartamentoSelecionado] = useState<string | null>(deptoUrl || null)
  const [cargoSelecionado, setCargoSelecionado] = useState<string | null>(cargoUrl || null)
  ...
  const filtros: FiltrosFuncionarios = {
    ...
    status: statusSelecionado && statusSelecionado !== 'all' ? statusSelecionado : undefined,
    departamentoId: departamentoSelecionado && departamentoSelecionado !== 'all' ? departamentoSelecionado : undefined,
    cargoId: cargoSelecionado && cargoSelecionado !== 'all' ? cargoSelecionado : undefined,
    pagina: paginaAtual,
    limite: 20,
  }
  ...
  const handleAlterarStatus = async (id: number, novoStatus: StatusFuncionarioType) => {
    const statusData = STATUS_FUNCIONARIO_DATA[novoStatus]
    if (confirm(`Deseja alterar o status do funcionário para "${statusData.label}"?`)) {
        ...
        <div className="space-y-6">
              ...
              <div className="flex flex-col gap-4 lg:flex-row lg:items-end">
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setStatusSelecionado(value as StatusFuncionarioType | 'all')}
                  >
                ...
                <div>
                  ...
                  <LookupSelect
                    ...
                    onValueChange={(val) => setCargoSelecionado(val && val !== 'all' ? val : null)}
                    options={
                      cargosData?.map((e) => ({
                        value: e.id.toString(),
                        label: e.nome,
                    ...
                    emptyMessage={isLoadingCargos ? 'Carregando...' : 'Nenhum cargo encontrado'}
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {isLoading ? 'Carregando...' : `${data?.total || 0} funcionários encontrados`}
              </CardDescription>
            </CardHeader>
            <CardContent>
              {isLoading ? (
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-muted-foreground mb-4">
                    {busca || statusSelecionado !== 'all' || departamentoSelecionado || cargoSelecionado
                    ...
                    <TableBody>
                      {data.funcionarios.map((funcionario) => (
                        <TableRow key={funcionario.id}>
                          ...
                          <TableCell className="font-mono text-sm">{formatCPF(funcionario.cpf)}</TableCell>
                          <TableCell>
                            <Badge variant="outline">{funcionario.cargo?.nome || 'N/A'}</Badge>
                          </TableCell>
                          <TableCell>
                            <Badge variant="outline">{funcionario.departamento?.nome || 'N/A'}</Badge>
                          </TableCell>
                          <TableCell className="font-mono text-sm">{formatPhone(funcionario.telefone)}</TableCell>
                          <TableCell>
                          ...
                          <TableCell>
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end">
                                <Link to="/admin/rh/funcionarios/$id" params={{ id: funcionario.id.toString() }}>
                                  <DropdownMenuItem>
                                    <Eye className="h-4 w-4 mr-2" />
                                    Visualizar
                                  </DropdownMenuItem>
                                </Link>
                                <Link to="/admin/rh/funcionarios/$id/edit" params={{ id: funcionario.id.toString() }}>
                                  <DropdownMenuItem>
                                    <Pencil className="h-4 w-4 mr-2" />
                                    Editar
                                  </DropdownMenuItem>
                                </Link>
                                {funcionario.status === STATUS_FUNCIONARIO.ATIVO && (
                                  <DropdownMenuItem
                                    onClick={() => handleAlterarStatus(funcionario.id, STATUS_FUNCIONARIO.AVISO_PREVIO)}
                                ...
                                {funcionario.status === STATUS_FUNCIONARIO.PERIODO_EXPERIENCIA && (
                                  <DropdownMenuItem
                                    onClick={() => handleAlterarStatus(funcionario.id, STATUS_FUNCIONARIO.ATIVO)}
                                ...
                                {[STATUS_FUNCIONARIO.AVISO_PREVIO, STATUS_FUNCIONARIO.EM_CONTRATACAO].includes(funcionario.status) && (
                                  <DropdownMenuItem
                                    onClick={() => handleAlterarStatus(funcionario.id, STATUS_FUNCIONARIO.DESLIGADO)}
