...
function RouteComponent() {
  ...
  // Mutations
  const { mutate: importarFuncionarios, isPending: importandoFuncionarios } = useMutation({
  ...
  const { mutate: sincronizarAfastamentos, isPending: sincronizandoAfastamentos } = useMutation({
    ...
    <RouteGuard requiredRoles={[USER_ROLES.ADMIN, USER_ROLES.GERENCIA_RH]}>
      <AdminLayout header={header}>
        <div className="space-y-6">
          ...
          {!mostrarApenasResultado && (
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <Button onClick={executarImportacao} disabled={importandoFuncionarios} className="w-full">
                  {importandoFuncionarios ? (
                    <RefreshCw className="h-4 w-4 mr-2 animate-spin" />
                  ) : (
                    <Download className="h-4 w-4 mr-2" />
                  )}
                  {importandoFuncionarios ? 'Importando...' : 'Importar Funcionários'}
                </Button>
              </CardContent>
            </Card>
          )}
          ...
          {!mostrarApenasResultado && (
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <Button onClick={executarSincronizacao} disabled={sincronizandoAfastamentos} className="w-full">
                  {sincronizandoAfastamentos ? (
                    <RefreshCw className="h-4 w-4 mr-2 animate-spin" />
                  ) : (
                    <RotateCcw className="h-4 w-4 mr-2" />
                  )}
                  {sincronizandoAfastamentos ? 'Sincronizando...' : 'Sincronizar Afastamentos'}
                </Button>
              </CardContent>
            </Card>
          )}
          ...
          {mostrarApenasResultado && !ultimoResultado && importandoFuncionarios && (
            <Card>
