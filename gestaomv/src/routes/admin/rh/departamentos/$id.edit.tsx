...
function RouteComponent() {
  ...
  const { data: departamento, isLoading } = useQuery(
    trpc.rh.obterDepartamento.queryOptions({ id: Number(id) })
  )

  const { mutate: atualizarDepartamento, isPending } = useMutation({
    ...trpc.rh.atualizarDepartamento.mutationOptions(),
    onSuccess: () => {
                ...
                <div className="pt-4 border-t">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <span className="ml-2 font-medium">{departamento._count?.funcionarios || 0}</span>
                    </div>
                    <div>
                      <span className="text-muted-foreground">Equipes:</span>
                      <span className="ml-2 font-medium">{departamento._count?.equipes || 0}</span>
                    </div>
                  </div>
                </div>
