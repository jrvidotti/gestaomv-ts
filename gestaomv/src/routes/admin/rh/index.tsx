...
function RouteComponent() {
  ...
  const { data: estatisticas, isLoading: carregandoStats } = useQuery(trpc.rh.obterEstatisticas.queryOptions(filtros))
  ...
  const { data: proximasAvaliacoes, isLoading: carregandoAvaliacoes } = useQuery(
    trpc.rh.obterProximasAvaliacoes.queryOptions({ limite: 5 })
  )
  ...
  const stats = [
    {
      ...
      value: carregandoStats ? '...' : estatisticas?.funcionariosAtivos?.toString() || '0',
    ...
    },
    {
      ...
      value: carregandoStats ? '...' : estatisticas?.equipesAtivas?.toString() || '0',
    ...
    {
      title: 'Departamentos',
      value: carregandoStats ? '...' : estatisticas?.totalDepartamentos?.toString() || '0',
    ...
    },
    {
      ...
      value: carregandoStats ? '...' : estatisticas?.avaliacoesPendentes?.toString() || '0',
  ...
  const dadosStatus = estatisticas
    ? [
        { name: 'Ativos', value: estatisticas.funcionariosPorStatus.ativo, color: '#00C49F' },
        { name: 'Período Experiência', value: estatisticas.funcionariosPorStatus.periodoExperiencia, color: '#FFBB28' },
        { name: 'Em Contratação', value: estatisticas.funcionariosPorStatus.emContratacao, color: '#0088FE' },
        { name: 'Aviso Prévio', value: estatisticas.funcionariosPorStatus.avisoPrevio, color: '#FF8042' },
      ].filter((item) => item.value > 0)
    : []

  // Dados para gráfico de departamentos
  const dadosDepartamentos = departamentos?.map((dept, index) => ({
    name: dept.nome.length > 15 ? dept.nome.substring(0, 15) + '...' : dept.nome,
        ...
        <div className="space-y-6">
            ...
            <Link to="/admin/rh/funcionarios">
              <Card className="hover:bg-accent cursor-pointer transition-colors">
                <CardHeader>
                  ...
                  <CardDescription>Gerenciar funcionários da empresa</CardDescription>
                </CardHeader>
                <CardContent>
            ...
            <Link to="/admin/rh/equipes">
              <Card className="hover:bg-accent cursor-pointer transition-colors">
                <CardHeader>
                  ...
                  <CardDescription>Organizar equipes de trabalho</CardDescription>
                </CardHeader>
                <CardContent>
                  <Button variant="ghost" className="w-full">
                    Ver Equipes
            ...
            <Link to="/admin/rh/avaliacoes-experiencia">
              <Card className="hover:bg-accent cursor-pointer transition-colors">
                <CardHeader>
                  ...
                  <CardDescription>Avaliar desempenho dos funcionários</CardDescription>
                </CardHeader>
                <CardContent>
                  <Button variant="ghost" className="w-full">
                    Ver Avaliações
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Funcionários que precisam ser avaliados</CardDescription>
            </CardHeader>
            <CardContent>
              {carregandoAvaliacoes ? (
                ...
                <div className="space-y-4">
                  {proximasAvaliacoes.map((avaliacao) => (
                    <div key={avaliacao.funcionarioId} className="flex items-center justify-between p-4 border rounded-lg">
                      <div className="flex items-center gap-3">
                        <div className="flex items-center justify-center w-10 h-10 bg-orange-100 text-orange-600 rounded-full">
                          <Clock className="h-5 w-5" />
                        </div>
                        <div>
                          <h4 className="font-semibold">{avaliacao.funcionarioNome}</h4>
                          <p className="text-sm text-muted-foreground">
                            {avaliacao.tipo === 'EXPERIENCIA' ? 'Avaliação de Experiência' : 'Avaliação Periódica'}
                          </p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="text-sm font-medium">
                          {new Date(avaliacao.dataVencimento).toLocaleDateString('pt-BR')}
                        </p>
          ...
          {!carregandoStats && estatisticas && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>Visualizar funcionários por situação</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="grid gap-4 md:grid-cols-4">
                  <Link to="/admin/rh/funcionarios" search={{ status: STATUS_FUNCIONARIO.ATIVO }}>
                    <div className="text-center p-4 border rounded-lg hover:bg-accent cursor-pointer transition-colors">
                      ...
                      <p className="text-2xl font-bold">{estatisticas.funcionariosPorStatus.ativo}</p>
                      <p className="text-sm text-muted-foreground">Ativos</p>
                    </div>
                  </Link>
                  <Link to="/admin/rh/funcionarios" search={{ status: STATUS_FUNCIONARIO.PERIODO_EXPERIENCIA }}>
                    <div className="text-center p-4 border rounded-lg hover:bg-accent cursor-pointer transition-colors">
                      <Clock className="h-8 w-8 mx-auto mb-2 text-yellow-500" />
                      <p className="text-2xl font-bold">{estatisticas.funcionariosPorStatus.periodoExperiencia}</p>
                      <p className="text-sm text-muted-foreground">Em Experiência</p>
                    </div>
                  </Link>
                  <Link to="/admin/rh/funcionarios" search={{ status: STATUS_FUNCIONARIO.EM_CONTRATACAO }}>
                    <div className="text-center p-4 border rounded-lg hover:bg-accent cursor-pointer transition-colors">
                      <Users className="h-8 w-8 mx-auto mb-2 text-blue-500" />
                      <p className="text-2xl font-bold">{estatisticas.funcionariosPorStatus.emContratacao}</p>
                      <p className="text-sm text-muted-foreground">Em Contratação</p>
                    </div>
                  </Link>
                  <Link to="/admin/rh/funcionarios" search={{ status: STATUS_FUNCIONARIO.AVISO_PREVIO }}>
                    <div className="text-center p-4 border rounded-lg hover:bg-accent cursor-pointer transition-colors">
                      <UserX className="h-8 w-8 mx-auto mb-2 text-red-500" />
                      <p className="text-2xl font-bold">{estatisticas.funcionariosPorStatus.avisoPrevio}</p>
