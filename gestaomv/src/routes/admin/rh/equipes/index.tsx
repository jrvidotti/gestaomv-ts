...
import { Building2, Plus, Search, MoreHorizontal, Pencil, Eye, Users, Trash2, Filter } from 'lucide-react'
...
function RouteComponent() {
  ...
  const { busca: buscaUrl, departamentoId: deptoUrl, pagina: paginaUrl } = Route.useSearch()
  ...
  const { data, isLoading, refetch } = useQuery(trpc.rh.listarEquipes.queryOptions(filtros))
  ...
  const { mutate: removerEquipe } = useMutation({
    ...trpc.rh.removerEquipe.mutationOptions(),
    onSuccess: () => {
      refetch()
    },
  })
  ...
  const handleRemover = async (id: number, nome: string) => {
    if (confirm(`Deseja realmente remover a equipe "${nome}"? Esta ação não pode ser desfeita.`)) {
      removerEquipe({ id })
    }
  }
        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Use os filtros abaixo para encontrar equipes específicas</CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {isLoading ? 'Carregando...' : `${data?.total || 0} equipes encontradas`}
              </CardDescription>
            </CardHeader>
            <CardContent>
              {isLoading ? (
                    ...
                    <TableBody>
                      {data.equipes.map((equipe) => (
                        <TableRow key={equipe.id}>
                          ...
                          <TableCell>
                            <Badge variant="outline">{equipe.departamento?.nome || 'N/A'}</Badge>
                          </TableCell>
                          <TableCell>
                            <div className="flex items-center gap-2">
                              <Users className="h-4 w-4 text-muted-foreground" />
                              <span className="text-sm">{equipe.membros?.length || 0} membros</span>
                            </div>
                          </TableCell>
                          <TableCell>
                          ...
                          <TableCell>
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end">
                                <Link to="/admin/rh/equipes/$id" params={{ id: equipe.id.toString() }}>
                                  <DropdownMenuItem>
                                    <Eye className="h-4 w-4 mr-2" />
                                    Visualizar
                                  </DropdownMenuItem>
                                </Link>
                                <Link to="/admin/rh/equipes/$id/edit" params={{ id: equipe.id.toString() }}>
                                  <DropdownMenuItem>
                                ...
                                <DropdownMenuItem
                                  onClick={() => handleRemover(equipe.id, equipe.nome)}
