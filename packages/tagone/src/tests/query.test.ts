...
const expectedResult =
  `?$expand=PessoaTelefones($select=CodigoPessoaTelefone,Telefone,Situacao;$filter=CodigoTipoContato eq 1 and Telefone eq '${nro_telefone}';$orderby=Situacao desc)&$select=DataNascimento,CpfCnpj,NomePessoa,CodigoPessoa&$filter=CpfCnpj eq '${cpf}'`;
...
test("mountOdataQuery ok 2", async () => {
  const query: OdataQuery = {
    $expand:
      `PessoaTelefones($select=CodigoPessoaTelefone,Telefone,Situacao;$filter=CodigoTipoContato eq 1 and Telefone eq '${nro_telefone}';$orderby=Situacao desc)`,
    $select: "DataNascimento,CpfCnpj,NomePessoa,CodigoPessoa",
    $filter: `CpfCnpj eq '${cpf}'`,
  };
